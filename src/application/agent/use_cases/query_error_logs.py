...
class QueryErrorLogsUseCase:
    ...
    def execute(self, query_dto: LogQueryDTO, account: str, region: str) -> LogSummaryDTO:
        """Query logs and publish error events"""
        try:
            # Read logs from CloudWatch
            log_entries = self._query_logs(query_dto)
            ...
            if error_logs:
                self._publish_error_events(error_logs, account, region)
    ...
    def _query_logs(self, query_dto: LogQueryDTO) -> List[LogEntry]:
    ...
    def _publish_error_events(self, error_logs: List[LogEntry], account: str, region: str) -> None:
