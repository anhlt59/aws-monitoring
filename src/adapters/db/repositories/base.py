...
class DynamoRepository[M: DynamoModel]:
    ...
    async def _get(self, hash_key: Any, range_key: Any = None, attributes_to_get: List[str] | None = None) -> M:
        def _sync_get():
            try:
    ...
    async def _create(self, model: M):
        def _sync_create():
    ...
    async def _update(self, hash_key: Any, range_key: Any = None, attributes: dict | None = None):
        def _sync_update():
            # build actions
            actions = []
    ...
    async def _delete(self, hash_key: Any, range_key: Any = None):
        def _sync_delete():
            # raise error if item does not exist
            condition = self.hash_key_attr.exists()
            if self.range_key_attr is not None:
