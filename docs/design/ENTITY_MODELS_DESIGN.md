...
class Task(BaseModel):
    ...
    assigned_user: dict               # {"id": str, "name": str}
...
```python
class AwsConfigStatus(str, Enum):
    """AWS Config deployment/monitoring status."""
...
class AwsConfig(BaseModel):
    ...
    AWS credentials are stored in monitoring profile, not in this model.
    """
    ...
    role_arn: str | None = None  # IAM role ARN for cross-account access (optional)

    # Deployment/Monitoring Status (previously Agent fields)
    status: AwsConfigStatus = AwsConfigStatus.PENDING
...
class AwsConfigPersistence(DynamoModel, discriminator="AWS"):
    pk = KeyAttribute(hash_key=True, default="CONFIG")
    sk = KeyAttribute(range_key=True, prefix="AWS#")
    ...
    account_sk = KeyAttribute()  # AWS#{id}
```
...
## 4. Complete DynamoDB Single Table Design
...
|--------|----|----|---------|---------|---------|---------|
| **Event** | EVENT | EVENT#{timestamp}#{id} | - | - | SOURCE#{source} | EVENT#{timestamp}#{id} |
| **User** | USER | USER#{id} | EMAIL#{email} | USER#{id} | ROLE#{role} | USER#{id} |
| **Task** | TASK | TASK#{id} | ASSIGNED#{user_id} | STATUS#{status}#TASK#{id} | STATUS#{status} | CREATED#{created_at}#TASK#{id} |
| **AwsConfig** | CONFIG | AWS#{id} | ACCOUNT_ID#{account_id} | AWS#{id} | - | - |
...
# Get AWS config by ID
pk = "CONFIG", sk = "AWS#{id}"

# Get AWS config by account_id (GSI1)
GSI1-PK = "ACCOUNT_ID#{account_id}", GSI1-SK begins_with "AWS#"
...
# Config DTOs
class CreateAwsConfigDTO(BaseModel):
    account_id: str
    account_name: str
    region: str
...
### 5.2 Output Models (Query Results)
...
**Note:**
- Agent model has been removed - its functionality is now part of AwsConfig entity
- TaskComment is now nested within Task model as an array, not a separate entity
- AWS credentials are stored in monitoring profile, not in AwsConfig
