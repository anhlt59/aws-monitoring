...
from src.entrypoints.functions.query_error_logs.main import handler, log_service
...
def mock_cloudwatch_logs(log_group_name: str):
    ...
    # Mock the log_service
    log_service.ecs_service.list_clusters = MagicMock(side_effect=lambda *a, **kw: [])
    log_service.lambda_service.list_functions = MagicMock(
        side_effect=lambda *a, **kw: [{"Tags": {"monitoring": "true"}, "LoggingConfig": {"LogGroup": log_group_name}}]
    )
